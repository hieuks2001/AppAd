...
<div class="overflow-x-auto bg-white drop-shadow-2xl p-5 rounded-2xl mt-10">
  ...
  <div class="btn-group flex justify-center mt-5">
    <a class="btn btn-outline btn-sm {{$statistical->onFirstPage() ? 'btn-disabled' : ''}}"
      href="{{$statistical->previousPageUrl()}}">Previous</a>
    <a class="btn btn-outline btn-sm {{!$statistical->hasMorePages() ? 'btn-disabled' : ''}}"
      href="{{$statistical->nextPageUrl()}}">Next</a>
  </div>
</div>
@endsection
